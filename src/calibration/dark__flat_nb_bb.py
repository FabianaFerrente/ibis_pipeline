...
# This script computes the average dark image from BB_Dark FITS files, filters outliers,
# and saves the result with an updated FITS header. It also generates plots of mean pixel
# values and RMS over time.
# The script assumes that the input files are in a specific format and contain valid data cubes.
# The function `avg_dark_bb` takes a directory containing the FITS files, an optional
# outlier sigma threshold, and options for saving the output FITS file and generating plots.
# The function `time_to_seconds` converts a time string in the format HH:MM:SS.sss into seconds.
# The script uses the `astropy` library for handling FITS files and `matplotlib` for plotting.
